...
fn get_missing_dependencies(exec_path: &Path) -> anyhow::Result<Vec<String>> {
    ...
    let missing_deps = if output.status.success() {
        output
            ...
            .filter_map(|l| l.split(' ').next().map(|s| s.trim_start().to_string()))
            .collect()
    } else {
        Vec::new()
    };
...
fn main() -> anyhow::Result<()> {
    ...
    // Check packages
    let missing_deps: Vec<(Arc<String>, Arc<PathBuf>, String)> = packages
        .into_par_iter()
